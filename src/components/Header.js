  ...
  return (
    <header className='fixed w-full px-[30px] lg:px-[100px] z-30 h-[100px] lg:h-[140px] flex items-center'>
      <div className='flex flex-col lg:flex-row lg:items-center w-full justify-between'>
        {/* logo  */}
        <Link to='/' className='max-w-[200px]' onMouseEnter={mouseEnterHandler} onMouseLeave={mouseLeaveHandler}>
          <img src={Logo} alt='' />
        </Link>
        {/* nav - initially hidden - show on desptop mode */}
        <nav className="hidden xl:flex gap-x-12 font-semibold" onMouseEnter={mouseEnterHandler} onMouseLeave={mouseLeaveHandler}>
